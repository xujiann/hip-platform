...
矩阵收官快赢包（部分响应 8 条升正偏离，docs/验收/部分响应筛选.md）；V44：

- 住院每日费用清单（按执行日期检索明细与合计，出院结算页内嵌）
- 结账明细检索加项目类型条件（明细行模式）
- 医生排班上一周/下一周快捷切换
- 审方工作台单次待审列表上限（`review_pending_limit`，0=不限）
- 单据编号前缀配置化（`billno_prefix_*` 五键：SJ/ZY/CY/CF/SQ，默认与既有一致）
- 输血不良反应处置方案字典（4 项种子，输血记录下拉留痕）
- 死亡登记卡（登记/列表/打印数据集，必填拦截 9950）
- 体检套餐「不进入总检结果」项目配置（`hidden_items`，接口层达成；套餐维护 UI 随体检深化）
- 质量：58 测试（+3）、20 套 E2E（新增 e2e-101 进 CI）
