...
MANUAL = {
    ...
    # v49：4943/4944 的消息里带动态拼接的取值清单，正则会把同一码的多条消息硬接成一串乱码
    # （实测速查表里 4944 变成「分桶取值非法：分桶取值非法：…」四段叠加，一线按码查到没法用）。
    # 手工登记覆盖。
    '4943': ('事件类指标的时间归集口径参数非法（by 只接受 event / surgery；'
             'event=按事件发生时间，surgery=v46 旧口径按手术锚点日期）', '麻醉质控'),
    '4944': ('穿透明细的分桶取值非法（bucket 须与该指标汇总里的分桶列取值逐字一致；'
             '非构成比类指标不支持分桶）', '麻醉质控'),
...
def collect():
    ...
    for code, (msg, mod) in MANUAL.items():
        # **手工登记覆盖而非追加**：登记的目的就是取代抓不准的自动结果。
        # 此前是 add()，于是 4944 这种「消息里带动态拼接取值清单」的码
        # 会把正则抓到的四段残句与手工文案叠在一起，速查表里成了
        # 「分桶取值非法：分桶取值非法：…」——一线按码查到根本没法用。
        hits[int(code)] = {(msg, mod + '（手工登记）')}
    return hits
