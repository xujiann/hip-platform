...
if sid:
    ...
    # —— 拒收的两条连带红线（复核发现、实测复现后修，V145 + 既有端点加条件）——
    # ① **已拒收的标本不得再核收、不得出报告**：v48 的拒收刻意不改 status，
    #    既有 receive/diagnose 原本不知道「拒收」存在，实测可走通
    #    「拒收 → 核收 → 诊断」，台账上出现「已拒收却已发报告」。
    rbc = rej.get('barcode')
    assert rbc, f'登记返回体应带条码：{rej}'
    assert call('PUT', f'/pathology/specimens/{rbc}/receive', {}, t)['code'] == 4551,         '**已拒收的标本不得再核收**——否则拒收后仍能一路走到发报告，台账自相矛盾'
    # ② **拒收后同一部位必须能重新登记**：拒收后临床重送是常态
    #    （固定液不对 / 离体过久 / 标本量不足都会拒）。唯一索引若不排除已拒收行，
    #    这个部位序号就被永久烧掉、只能顺延，而部位序号正是蜡块编码与
    #    报告上「3 号蜡块」的来源——那样拒收功能反而成了给自己下的绊子。
    again = call('POST', '/pathology/registry/specimens',
                 {'orderId': oid, 'partNo': 3, 'specimenType': 'ROUTINE',
                  'specimenDesc': '重送标本'}, t)
    assert again['code'] == 0,         f'**拒收后同一部位必须能重新登记**（临床重送是常态，拒收不能等于把部位序号烧掉）：{again}'
    print('[3] 拒收 OK（原因必填 / **不删行** / **已拒收不得再核收** / **同部位可重送登记**）')
