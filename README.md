...
**当前状态（2026-09-17）**：`v1.6.4` · 迁移 V168 · 13 后端模块 + 105 前端页面 ·
1070 单元/集成测试 + 46 套 E2E（全新库 CI 顺序全绿，含演示驱动脚本）· 错误码 540（速查表全量） · sys_config 98 键 · 六轮全库审阅闭环 ·
[性能基线 v2](docs/性能基线-20260818.md)（10 万级实测）· 投标应答核账进行中（v3 偏离表已上调 2 条，待签认见 `docs/验收/偏离表重出-签认清单.md`）。
...
mvn test          # 1070 单元/集成测试；E2E 见 tools/e2e-*.py（需后端运行）
```
