...
public class OutpAppointmentController {
    ...
    public R<List<Map<String, Object>>> slots(@PathVariable Long scheduleId) {
        return R.ok(jdbc.queryForList("""
                ...
                from outp_schedule_slot where schedule_id = ? order by time_begin
                """, scheduleId));
    }
    ...
    public R<Map<String, Object>> book(@RequestBody ApptReq req) {
        var slot = jdbc.queryForList(
                "select schedule_id from outp_schedule_slot where id = ? and enabled", req.slotId());
        if (slot.isEmpty()) return R.fail(3110, "时段不存在或已停用");
        ...
        Integer dup = jdbc.queryForObject(
                ...
                Integer.class, scheduleId, req.patientId());
        if (dup != null && dup > 0) return R.fail(3112, "该患者此排班已有有效预约");
        ...
        Integer regNo = jdbc.queryForObject("select booked from outp_schedule where id = ?", Integer.class, scheduleId);
        try {
            ...
            jdbc.update(con -> {
                var ps = con.prepareStatement("""
                        ...
                        values (?,?,?,?,?)
                        """, new String[]{"id"});
                ps.setLong(1, req.slotId());
                ps.setLong(2, scheduleId);
                ps.setLong(3, req.patientId());
                ps.setInt(4, regNo == null ? 0 : regNo);
                ps.setString(5, req.source());
                return ps;
            }, kh);
            return R.ok(Map.of("id", kh.getKey().longValue(), "apptNo", regNo));
    ...
    public R<Void> cancel(@PathVariable Long id) {
        var rows = jdbc.queryForList(
                "select slot_id, schedule_id from outp_appointment where id = ?", id);
        if (rows.isEmpty()) return R.fail(3113, "预约不存在");
        if (jdbc.update("update outp_appointment set status = 'CANCELLED', cancelled_at = now() "
                + "where id = ? and status = 'BOOKED'", id) == 0) {
            return R.fail(3114, "预约状态不允许取消（已签到或已取消）");
        }
