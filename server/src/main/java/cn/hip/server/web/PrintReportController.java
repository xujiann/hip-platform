...
public class PrintReportController {
    ...
    public R<List<Map<String, Object>>> auditLogs(@RequestParam(required = false) String username) {
        return R.ok(username == null
                ? jdbc.queryForList("select * from sys_audit_log order by id desc limit 200")
                : jdbc.queryForList("select * from sys_audit_log where username = ? order by id desc limit 200", username));
    }
}
