    ...
    <el-tabs v-model="tab">
      <el-tab-pane label="维修工单" name="repair">
        <el-form inline size="small">
          ...
          <el-button type="primary" size="small" @click="report">报修</el-button>
        ...
        <el-table :data="repairs" size="small" border>
          ...
          <el-table-column label="操作" width="150">
            <template #default="{ row }">
              <el-button v-if="row.status === 'REPORTED'" link type="primary" size="small" @click="assign(row)">派工</el-button>
              <el-button v-if="row.status === 'ASSIGNED'" link type="success" size="small" @click="doneRepair(row)">完成</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="保养与计量" name="maintain">
        <el-form inline size="small">
          ...
          <el-button type="primary" size="small" @click="addPlan">建保养计划</el-button>
        </el-form>
        <el-table :data="plans" size="small" border>
          ...
          <el-table-column label="操作" width="100">
            <template #default="{ row }">
              <el-button link type="success" size="small" @click="execute(row)">执行保养</el-button>
            </template>
          </el-table-column>
        </el-table>
        <h4>计量台账
          <el-button link type="primary" size="small" @click="addMetrologyDlg">登记检定</el-button>
        </h4>
      ...
      <el-tab-pane label="采购单据" name="purchase">
        <el-form inline size="small">
          ...
          <el-button type="primary" size="small" @click="createDoc">开单</el-button>
        </el-form>
        <el-table :data="docs" size="small" border>
          ...
          <el-table-column label="操作" width="220">
            <template #default="{ row }">
              <el-button v-if="row.status === 'DRAFT'" link type="primary" size="small" @click="docAction(row, 'approve')">审核</el-button>
              <el-button v-if="row.status === 'DRAFT'" link type="danger" size="small" @click="docAction(row, 'cancel')">作废</el-button>
              <el-button v-if="['APPROVED', 'RECEIVED'].includes(String(row.status))" link type="success" size="small"
                         @click="receiveDoc(row)">{{ row.status === 'RECEIVED' ? '改发票' : '验收' }}</el-button>
              <el-button v-if="row.status === 'CANCELLED'" link size="small" @click="docAction(row, 'restore')">还原</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="供应商证照" name="certs">
        <el-form inline size="small">
          ...
          <el-button type="primary" size="small" @click="addCert">登记</el-button>
        </el-form>
      ...
      <el-tab-pane label="制度文件库" name="doclib">
        <el-form size="small">
          ...
          <el-button type="primary" size="small" @click="uploadDoc">上传制度</el-button>
...
async function doneRepair(row: Record<string, unknown>) {
  const { value } = await ElMessageBox.prompt('维修结果', '完成维修', { inputValue: '更换配件后恢复正常' })
  await client.put(`/equipment/repairs/${row.id}/done`, null, { params: { note: value } })
  await loadRepairs()
}
async function addPlan() {
  if (!plan.assetId || !plan.nextDate) return
...
async function execute(row: Record<string, unknown>) {
  const { value } = await ElMessageBox.prompt('保养内容', '执行保养', { inputValue: '例行保养，运行正常' })
...
async function addMetrologyDlg() {
  const { value } = await ElMessageBox.prompt('资产ID,证书号,检定日,有效期至（逗号分隔）', '登记检定',
    { inputValue: '1,JD-2026-001,2026-08-01,2027-08-01' })
...
async function createDoc() {
  if (!doc.supplierId || !doc.items) return
